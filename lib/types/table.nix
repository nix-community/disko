...
let
  deprecationWarn =
if !config.silenceDeprecationWarning
  then lib.warn ''
          ...
          Or alternatively you can disable this warning if you're really, really, REALLY sure by setting:
  ...
  options =
    {
      ...
      silenceDeprecationWarning = lib.mkOption {
        type = lib.types.bool;
        default = false;
        description = ''
          Disable the legacy table deprecation warning. In some cases it is impractical or impossible to
          repartition a drive to GPT. This option will silence the warning.
        '';
      };
