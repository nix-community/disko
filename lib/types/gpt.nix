          ...
          { name, ... }@partition:
          {
            options = {
              ...
              uuid = lib.mkOption {
                ...
                default = null;
                defaultText = "`null` - let sgdisk generate a UUID when creating the partition";
                ...
                description = ''
                  ...
                  When this is `null`, the UUID is generated by `sgdisk` when the partition is created and is left
                  unchanged if the partition already exists. You can generate a fixed UUID with the command `uuidgen -r`.
                '';
              };
              label = lib.mkOption {
                type = lib.types.str;
    ...
    _create = diskoLib.mkCreateOption {
      ...
      default = ''
        ...
        ${lib.concatMapStrings (
          partition:
          let
            uuidArg = lib.optionalString (partition.uuid != null) ''
              --partition-guid="${toString partition._index}:${partition.uuid}" \
            '';
            args = ''
              ${uuidArg}--change-name="${toString partition._index}:${partition.label}" \
