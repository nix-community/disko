...
import json
import sys
from typing import Any, Callable, Literal, Mapping, TypeGuard


def is_type(
    type_field: dict[str, Any]
) -> TypeGuard[dict[Literal["default", "description", "type"], Any]]:
...
def parse_field(
    containing_class: str, field_name: str, field: dict[str, Any]
) -> tuple[str, io.StringIO | None]:
...
def transform_dict_keys(
    d: dict[str, Any], transform_fn: Callable[[str], str]
) -> dict[str, Any]:
    if not isinstance(d, Mapping):
        return d

    return {transform_fn(k): transform_dict_keys(v, transform_fn) for k, v in d.items()}


def generate_python_code(schema: dict[str, dict[str, Any]]) -> io.StringIO:
